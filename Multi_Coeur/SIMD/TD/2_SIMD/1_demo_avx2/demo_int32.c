...
__attribute__((noinline)) void minmax(int32_t *X, int32_t *Y, int32_t *min, int32_t *max) {
	__m256i reg_x, reg_y, reg_min, reg_max, mask;

	reg_x = _mm256_loadu_si256((__m256i *)X);
	reg_y = _mm256_loadu_si256((__m256i *)Y);

	mask = _mm256_cmpgt_epi32(reg_x, reg_y);

	reg_min = _mm256_blendv_epi8(reg_x, reg_y, mask);
	reg_max = _mm256_blendv_epi8(reg_y, reg_x, mask);

	_mm256_storeu_si256((__m256i *)max, reg_max);
	_mm256_storeu_si256((__m256i *)min, reg_min);
}
...
{
	{
		...
		while (i < argc)
		{
			...
			{
				printf("minmax\n\n");
				int32_t X[8] = {5, 8, 5, 1, 5, 0, 2, 8};
				int32_t Y[8] = {8, 0, 5, 4, 1, 9, 5, 3};
				int32_t *min = malloc(sizeof(int32_t) * REG_NB_ELEMENTS);
				int32_t *max = malloc(sizeof(int32_t) * REG_NB_ELEMENTS);
