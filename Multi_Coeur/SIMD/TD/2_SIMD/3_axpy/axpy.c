...
__attribute__((noinline)) void axpy (const float alpha, const float *x, const float *y, float *z, const int vector_size)
{
	...
	alpha_vec = _mm256_set1_ps(alpha);

	for (int i = 0; i < vector_size; i += REG_NB_ELEMENTS) {
		x_vec = _mm256_load_ps(x + i);
		y_vec = _mm256_load_ps(y + i);

		_mm256_store_ps(z + i, _mm256_fmadd_ps(alpha_vec, x_vec, y_vec));
	}
}
...
int main(int argc, char *argv[])
{
	...
	float alpha = ((float)((rand() % 42)-21)) / 2.0;
